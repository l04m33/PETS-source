...
CREATE TABLE buddy_extra (
    buddy_uuid char(36) references buddies(uuid),
    name varchar(512),
    value varchar(512),
    constraint pk_buddy_extra primary key (buddy_uuid, name)
);
...
CREATE TABLE ext_prefs (
    ext_id integer references exts(id),
    name varchar(512),
    value varchar(512),
    type varchar(256),
    constraint pk_ext_prefs primary key (ext_id, name)
);
...
CREATE TABLE exts (
    ...
    name varchar(256),
    path varchar(512),
    signature integer unsigned
);

CREATE TABLE file_tag (
    file_hash varchar(256) references local_files(hash),
    tag_id integer references tags(id),
    constraint pk_file_tag primary key (file_hash, tag_id)
);
...
CREATE TABLE local_files (
    ...
    seg_size bigint unsigned not null,
    mod_time date, size bigint
);

CREATE TABLE local_segs (
    hash varchar(256) primary key,
